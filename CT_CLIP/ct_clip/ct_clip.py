...
class CTCLIP(nn.Module):
    ...
    def forward(
        ...
        text_latents = rearrange(text_latents, '(m b) ... -> m b ...', m = 1) #NOTE: 1xbxd
        image_latents = rearrange(image_latents, '(m b) ... -> m b ...', m = 1) #NOTE: 1xbxd
        ...
        if self.use_all_token_embeds:
            ...
            text_to_image_mask = rearrange(text_mask, '(m b) t -> m 1 b 1 t', m = 1).bool()
            text_to_image = masked_mean(text_to_image, text_to_image_mask, dim = -1)

            image_to_text_mask = rearrange(text_mask, '(m b) t -> m 1 b 1 t 1', m = 1).bool()
