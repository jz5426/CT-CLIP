...
class MedKlipVisionModelResNet(nn.Module):
    '''
    ...
    def __init__(self, medclip_checkpoint=None):
        ...
        if medclip_checkpoint is not None:
            self.load_from_medklip(medclip_checkpoint)
        else:
            print('NOT LOADING ANY MEDICAL RELATED PRETRAINED WEIGHTS')
        
    def load_from_medklip(self, checkpoint):
...
class MedKlipVisionModel(nn.Module):
    def __init__(self,
        vision_cls=MedKlipVisionModelResNet,
        checkpoint=None,
        ) -> None:
        super().__init__()
        assert vision_cls in [MedKlipVisionModelResNet], 'vision_cls should be one of [MedCLIPVisionModel, MedCLIPVisionModelViT]'
