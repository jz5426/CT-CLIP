...
NOTE: look through the metadata like spacing and etc from the spreadsheets
"""
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith('.npz') and not file.startswith('._'):
                ...
                total+=1
    print('total number of qualified file: ', total)
...
def process_file(file_path, shared_dst_dir):
    """
    ...
    xray_save_folder = "preprocessed_xray_mha" #save folder for preprocessed
    xray_folder_path_new = os.path.join(shared_dst_dir, xray_save_folder)
    ...
    xray_rgb_save_folder = "preprocessed_xray_rgb" #save folder for preprocessed
    xray_folder_path_new = os.path.join(shared_dst_dir, xray_rgb_save_folder)
    os.makedirs(xray_folder_path_new, exist_ok=True)
    ...
    row = df[df['VolumeAcc_DEID'] == original_file_name]
    slope = float(row["orig_slope"].iloc[0])
    intercept = float(0)
    xyz_spacing = float(row["final_spacing"].iloc[0])
